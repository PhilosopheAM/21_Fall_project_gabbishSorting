...
void BMI088::ReadRegister(uint8_t _address, uint8_t* _pData, uint32_t _len, BMI088Cs cs)
{
    ...
    bsp_spi_send(SPI1,&_address,1,50);
    uint32_t _timeout = _len * 50;
    bsp_spi_receive(SPI1,_pData,_len,_timeout);
...
void BMI088::WriteRegister(uint8_t _address, uint8_t* _pData, uint32_t _len, BMI088Cs cs)
{
	...
	bsp_spi_send(SPI1, temp_buffer, 1 + _len, 50);
...
void BMI088::ReadData()
{
	for(int i = 0;i<6;i++)
	{
		ReadRegister(BMI088_ACCEL_XOUT_L+i,&acc_data[i],1,CS_Acc);
	}
    
    int16_t bmi088_raw_temp;

    bmi088_raw_temp = (int16_t)((acc_data[0]) << 8) | acc_data[1];
	m_az = (float)bmi088_raw_temp * 0.0008974358974f;

    bmi088_raw_temp = (int16_t)((acc_data[2]) << 8) | acc_data[3];
	m_ax = (float)bmi088_raw_temp * 0.0008974358974f;

    bmi088_raw_temp = (int16_t)((acc_data[4]) << 8) | acc_data[5];
