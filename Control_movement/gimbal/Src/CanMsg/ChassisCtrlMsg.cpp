...
void ChassisCtrlMsg::SendMsg()
{
    ...
    if(m_isAmmoLidOpen)
    {
        bitfieldBuffer |= kAmmoLidMask;
    }
    if(m_isSpinning)
    {
        bitfieldBuffer |= kSpinningMask;
    }
    if(m_isLowFireRate)
    {
        bitfieldBuffer |= kFireRateMask;
    }
    if(m_isFricOn)
    {
        bitfieldBuffer |= kFricMask;
    }

    bitfieldBuffer |= (m_gimbalFireMode << kGftOffset);
    bitfieldBuffer |= (m_aimbotStatus << kAstOffset);
